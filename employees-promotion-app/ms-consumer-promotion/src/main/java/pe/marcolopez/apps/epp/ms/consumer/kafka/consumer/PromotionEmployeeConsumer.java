package pe.marcolopez.apps.epp.ms.consumer.kafka.consumer;
...
import pe.marcolopez.apps.epp.ms.consumer.webclient.EmployeeWebClientService;
...
public class PromotionEmployeeConsumer {
  ...
  public void consumePromotionByEmployee(ConsumerRecord<String, PromotionEmployeeEvent> record,
    ...
    employeeMono.zipWith(leaderMono)
        ...
        .subscribe(
            savedPromotion -> {
              log.info("### PromotionEmployeeEvent saved: {}", savedPromotion);
              ack.acknowledge();
            },
            error -> log.error("Error processing PromotionEmployeeEvent: {}", error.toString())
        );
  }
  ...
  public void consumePromotionByLeader(ConsumerRecord<String, PromotionLeaderEvent> record,
    ...
    employeeMono.zipWith(leaderMono)
        ...
        .subscribe(
            savedPromotion -> {
              log.info("### PromotionLeaderEvent saved: {}", savedPromotion);
              ack.acknowledge();
            },
            error -> log.error("Error processing PromotionLeaderEvent: {}", error.toString())
        );
  }
}